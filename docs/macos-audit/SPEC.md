...
- **(B2) `restic` → Cloudflare R2** з launchd daemon. Daily 03:00. Encrypted. Retention 7d/4w/6m. Targets (final, 2026-05-21): `~/.local/share/chezmoi`. Виключити: caches, node_modules, .venv, Docker volumes. (`~/projects` виключено — старі/архівні, активні проекти у git. `~/Documents` виключено — в iCloud Drive sync. Obsidian vault deferred — TCC restrictions на iCloud path. Додавати окремі targets коли з'являться irreplaceable не-iCloud артефакти.)
